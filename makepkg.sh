...
main() {
    ...
    display_message "Checking $makepkg_conf"
    ...
    if [ -s "$makepkg_conf" ]; then
        ...
        if [ "$verbose_mode" = true ]; then
            display_message "${GREEN}Makepkg configuration updated successfully!${NC}"
        fi
    else
        display_message "Error: $makepkg_conf not found or is empty."
    fi
}
...
is_j_value_present() {
    if sudo grep -qE '^\s*(#)?MAKEFLAGS\s*=\s*.*-j[0-9]+' "$makepkg_conf" && ! sudo grep -qE '^\s*(#)?MAKEFLAGS\s*=\s*""' "$makepkg_conf"; then
        display_message "Detected -j value in MAKEFLAGS of $makepkg_conf"
        return 0
    fi
    display_message "No -j value detected in $makepkg_conf"
    return 1
}
...
update_j_value() {
    ...
    display_message "Changing -j value in $makepkg_conf to -j$num_threads"
}
...
display_message() {
    ...
    local message="$1"
    local file_name="${YELLOW}$(basename "$makepkg_conf")${NC}"
